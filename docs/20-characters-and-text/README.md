...
- `char`、UTF-16コード単位、Unicodeコードポイントの違い
- 文字列の長さとUTF-8バイト数が一致しない理由
- ファイルやネットワークで文字エンコーディングを意識する理由
- テキストブロックで複数行文字列を読みやすく書く方法
...
この2つの値が同じとは限りません。見た目の文字、UTF-16コード単位、Unicodeコードポイントは分けて考えます。
...
ファイル保存やHTTP通信では、文字列がどのエンコーディングでバイト列になるかを明示すると、環境差による文字化けを避けやすくなります。
...
```bash
cd docs/20-characters-and-text/examples
...
docker compose exec -w /workspace/docs/20-characters-and-text/examples java mvn compile exec:java
```
