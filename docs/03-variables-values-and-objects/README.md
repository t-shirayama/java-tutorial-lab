...
本文中の短いコード例は、実行できる [VariablesAndObjectsApp.java](examples/src/main/java/lab/variablesandobjects/VariablesAndObjectsApp.java) にまとまっています。まずこのファイルを上から読み、次に本文の各節へ戻ると、断片的な説明が1つの流れとしてつながります。
...
```bash
cd docs/03-variables-values-and-objects/examples
...
docker compose exec -w /workspace/docs/03-variables-values-and-objects/examples java mvn compile exec:java
```
...
### finalなら中身も変わらないと思ってしまう

```java
final List<String> names = new ArrayList<>();
names.add("Java"); // これはできる
```

`final`は変数へ別の参照を代入できなくするだけです。オブジェクトの中身まで必ず不変にするわけではありません。

## 練習問題

### Level 1

基本型の値と参照型の値を変えて出力を確認してください。

### Level 2

同じオブジェクトを2つの変数から参照する例を追加してください。

### Level 3

`final`なListに要素を追加する例と再代入できない例を説明してください。

## 理解チェック

1. 基本型と参照型の違いは何ですか？
2. 2つの変数が同じオブジェクトを参照すると、どんなことが起きますか？
3. ローカル変数とフィールドの初期値の扱いはどう違いますか？
