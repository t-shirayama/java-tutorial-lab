...
- クラス、フィールド、メソッド、コンストラクタを説明できる
- staticとインスタンスメンバの違いを説明できる
- 小さな不変オブジェクトを設計できる
...
docker compose exec -w /workspace/docs/08-classes/examples java mvn compile exec:java
```
...
### 状態をどこからでも変更できる設計にしてしまう

setterを何でも用意すると、どこで値が変わったか追いにくくなります。可能ならコンストラクタで必要な値を受け取り、その後は変更しない設計を検討します。

## 練習問題

### Level 1

コンストラクタへ渡す値を変えて出力を確認してください。

### Level 2

メソッドを1つ追加し、オブジェクトの情報を表示してください。

### Level 3

不変オブジェクトとして扱える小さなクラスを設計してください。

## 理解チェック

1. フィールド、メソッド、コンストラクタの役割は何ですか？
2. `static`メンバとインスタンスメンバは何が違いますか？
3. 不変オブジェクトにはどんな利点がありますか？
