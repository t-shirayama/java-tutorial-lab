...
`Integer.parseInt`に数値として読めない文字列を渡すと、`NumberFormatException`が発生します。例外は11章で詳しく扱います。この章では、変換には失敗する場合がある、と覚えておけば十分です。
...
```bash
cd docs/04-integers-and-booleans/examples
...
docker compose exec -w /workspace/docs/04-integers-and-booleans/examples java mvn compile exec:java
```
...
割り算の値を変えて、整数除算の結果を確認してください。

### Level 2

`parseInt`で変換する文字列を増やしてください。

### Level 3

点数から合否を判定する小さなメソッドを作ってください。

## 理解チェック

1. `5 / 2`の結果はなぜ`2`になりますか？
2. `Integer.parseInt`で数字でない文字列を渡すと何が起きますか？
3. `boolean`の条件式はどのような場面で使いますか？
