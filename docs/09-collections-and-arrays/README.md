...
- List、Map、Set、ArrayDeque、配列を使い分けられる
- 変更可能コレクションと変更不可コレクションの違いを説明できる
- コレクションを繰り返し処理できる
...
```bash
cd docs/09-collections-and-arrays/examples
...
docker compose exec -w /workspace/docs/09-collections-and-arrays/examples java mvn compile exec:java
```
...
2. `Map.get()`で存在しないキーを指定すると何が返ることがありますか？
3. `List.of()`で作ったリストに`add()`するとどうなりますか？
