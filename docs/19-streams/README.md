...
Streamではラムダ式をよく使います。ラムダ式に不安がある場合は、先に[18章 メソッド参照とラムダ式](../18-method-references-and-lambdas/)を復習してください。
...
- コレクションを「絞り込む、変換する、集める」流れで処理する考え方
- 中間処理と終端処理の違い
- 数値ストリーム、並列ストリーム、Optionalの入口
- for文とStreamを目的に応じて選ぶ判断
...
本文中の短いコード例は、実行できる [StreamsApp.java](examples/src/main/java/lab/streams/StreamsApp.java) にまとまっています。まずこのファイルを上から読み、次に本文の各節へ戻ると、断片的な説明が1つの流れとしてつながります。
...
docker compose exec -w /workspace/docs/19-streams/examples java mvn compile exec:java
```

期待される出力例:

```text
long lessons: [コレクション, ストリーム]
total minutes: 85
first long: コレクション
```
...
`minutes >= 30`を別の条件に変えて結果を確認してください。

### Level 2

`map`でタイトルと時間を組み合わせた文字列へ変換してください。

### Level 3

30分以上の件数、合計時間、タイトル一覧を表示してください。
