...
```bash
docker compose exec -w /workspace/docs/22-practical-basics/examples java mvn compile exec:java
```

JUnitテストを実行します。

```bash
docker compose exec -w /workspace/docs/22-practical-basics/examples java mvn test
```

ローカルにJava 21とMavenを入れている場合は、次でも実行できます。

```bash
cd docs/22-practical-basics/examples
...
正常に動く入力だけでなく、`null`や不正な文字列など異常系も少しずつ確認します。
...
本文中の短いコード例は、実行できる [PracticalBasicsApp.java](examples/src/main/java/lab/practicalbasics/PracticalBasicsApp.java) にまとまっています。まずこのファイルを上から読み、次に本文の各節へ戻ると、断片的な説明が1つの流れとしてつながります。
