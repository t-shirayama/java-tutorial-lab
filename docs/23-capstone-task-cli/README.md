...
- これまでの章で学んだ型、コレクション、Stream、例外、ファイル操作の組み合わせ方
- CLIアプリをデータ、処理、保存に分けて読む方法
- テストしやすい処理を`TaskService`へ切り出す考え方
- 小さく動かしながら機能を追加する実装手順
...
ここでは[13章 データを表す型](../13-data-types/)で学んだ`record`を使い、データを表す型を短く定義します。

## Step 2: TaskStatus enumを作る

状態は`TODO`と`DONE`のように、決まった候補から選びます。文字列ではなく`enum`にすると、打ち間違いをコンパイル時に見つけやすくなります。

ここでは[13章 データを表す型](../13-data-types/)の`enum`を使い、状態の候補をコード上で固定します。

## Step 3: Listでタスクを管理する

複数のタスクは`List<Task>`として扱います。追加、一覧表示、検索の入口になります。

ここでは[9章 コレクションと配列](../09-collections-and-arrays/)で学んだ`List`を使い、複数の値を順序付きで扱います。
...
```bash
cd docs/23-capstone-task-cli/examples
...
docker compose exec -w /workspace/docs/23-capstone-task-cli/examples java mvn compile exec:java
```
...
```bash
docker compose exec -w /workspace/docs/23-capstone-task-cli/examples java mvn test
```
