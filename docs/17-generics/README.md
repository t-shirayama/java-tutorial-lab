...
`List<String>`や`Map<String, Integer>`は、[9章 コレクションと配列](../09-collections-and-arrays/)でも登場しました。この章では、型を指定する理由を詳しく見ます。
...
本文中の短いコード例は、実行できる [GenericsApp.java](examples/src/main/java/lab/generics/GenericsApp.java) にまとまっています。まずこのファイルを上から読み、次に本文の各節へ戻ると、断片的な説明が1つの流れとしてつながります。
...
docker compose exec -w /workspace/docs/17-generics/examples java mvn compile exec:java
```
