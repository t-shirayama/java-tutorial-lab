...
- Javaプログラムの基本構成を説明できる
- Maven標準構成のどこにコードを書くか判断できる
- 読みやすい名前と小さな役割分担を意識してコードを書ける
...
```java
String studyTopic = "Java";
System.out.println(studyTopic);
```

`studyTopic`のように、複数の単語をつなげる名前では2つ目以降の単語を大文字で始めます。この書き方をキャメルケースと呼びます。

セミコロン`;`は文の終わりを表します。波括弧`{ ... }`は処理のまとまりを表します。インデントはJavaの文法として必須ではありませんが、人間がブロックの対応を読み取るためにとても重要です。
...
サンプルの表示文言を1か所変えて実行してください。

### Level 2

学習ステップを1つ追加し、出力順がどう変わるか確認してください。

### Level 3

新しいクラスを1つ追加し、役割ごとにコードを分ける練習をしてください。
