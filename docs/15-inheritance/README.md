...
`protected`はサブクラスから使えるようにするアクセス修飾子ですが、広げすぎると親クラスの内部構造へ強く依存します。まずは`private`を基本にし、本当にサブクラスへ公開したい操作だけを`protected`にします。

## この章の全体コード例

本文中の短いコード例は、実行できる [InheritanceApp.java](examples/src/main/java/lab/inheritance/InheritanceApp.java) にまとまっています。まずこのファイルを上から読み、次に本文の各節へ戻ると、断片的な説明が1つの流れとしてつながります。
...
```bash
cd docs/15-inheritance/examples
...
docker compose exec -w /workspace/docs/15-inheritance/examples java mvn compile exec:java
```
...
is-a関係ではないのに継承すると、親子関係が強く結び付きます。単なる再利用なら委譲を検討します。
