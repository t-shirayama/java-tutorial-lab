...
- 複数スレッドが同じ値を更新すると起きる競合状態
- `count++`が「読む、足す、書く」に分かれること
- `AtomicInteger`や`synchronized`で更新を守る考え方
- 共有しない、変更しない、制御して共有するという設計の選択肢
...
本文中の短いコード例は、実行できる [ConcurrencyControlApp.java](examples/src/main/java/lab/concurrencycontrol/ConcurrencyControlApp.java) にまとまっています。まずこのファイルを上から読み、次に本文の各節へ戻ると、断片的な説明が1つの流れとしてつながります。
...
```bash
cd docs/25-concurrency-control/examples
...
docker compose exec -w /workspace/docs/25-concurrency-control/examples java mvn compile exec:java
```
