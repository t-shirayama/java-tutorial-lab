...
文字列比較の失敗は[7章](../07-strings/)、変更不可Listの失敗は[9章](../09-collections-and-arrays/)、例外設計は[11章](../11-exceptions/)、`null`やテストは[22章](../22-practical-basics/)でも扱います。
...
```bash
docker compose exec -w /workspace/docs/12-errors-ide-debugging/examples java mvn compile exec:java
```

ローカルにJava 21とMavenを入れている場合は、次でも実行できます。

```bash
cd docs/12-errors-ide-debugging/examples
...
- [3章 変数・値・オブジェクト](../03-variables-values-and-objects/)
- [22章 実務入口](../22-practical-basics/)
...
- [9章 コレクションと配列](../09-collections-and-arrays/)
...
- [4章 整数とブーリアン](../04-integers-and-booleans/)
...
本文中の短いコード例は、実行できる [ErrorsIdeDebuggingApp.java](examples/src/main/java/lab/errorside/ErrorsIdeDebuggingApp.java) にまとまっています。まずこのファイルを上から読み、次に本文の各節へ戻ると、断片的な説明が1つの流れとしてつながります。
