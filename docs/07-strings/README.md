...
変数や型の読み方に不安がある場合は、先に[3章 変数・値・オブジェクト](../03-variables-values-and-objects/)と[5章 文・式・演算子](../05-statements-expressions-and-operators/)を読むと理解しやすくなります。
...
docker compose exec -w /workspace/docs/07-strings/examples java mvn compile exec:java
```
...
2. 文字列の内容比較に`==`ではなく`equals`を使う理由は何ですか？
3. `StringBuilder`はどのような場面で役立ちますか？
