...
ファイル入出力では例外処理が必要になります。実際のファイル読み書きは[22章 実務入口](../22-practical-basics/)で扱います。
...
- エラー、例外、チェック例外、非チェック例外の違い
- `try`、`catch`、`finally`、try-with-resourcesの役割
- `throw`、`throws`、独自例外クラスの使いどころ
- 例外を握りつぶさず、呼び出し側へ意味を伝える設計
...
```bash
cd docs/11-exceptions/examples
...
docker compose exec -w /workspace/docs/11-exceptions/examples java mvn compile exec:java
```
